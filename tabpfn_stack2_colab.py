# =====================================================================
# Datathon 2026 - ZENGINLESTIRILMIS TabPFN meta-stacker (Colab L4)
# Girdi: bilesen tahminleri (bag'li) + TUM ham tabular feature + kategorik kodlar
#        + yil. TabPFN hem ham sinyali hem bilesen duzeltmesini birlikte ogrenir.
# STACK-BAG: 3 fold-seed. Cikti: stack2_oof.npy, stack2_test.npy (+ Drive).
# Kurulum: drive mount + !pip install -q tabpfn + train/test + stack_inputs.zip
#   !unzip -o stack_inputs.zip -d stack
#   Hucre: os.environ["TABPFN_TOKEN"]="..." + bu dosya
# =====================================================================
import os
import shutil
import numpy as np
...
# ham tabular (kategorikler kodlu) + bilesen tahminleri
...
Xtr = np.column_stack([np.column_stack(list(Ctr.values())), Rtr]).astype(np.float32)
Xte = np.column_stack([np.column_stack(list(Cte.values())), Rte]).astype(np.float32)
print("zenginlestirilmis stacker girdisi:", Xtr.shape)  # ~17 + 45 = 62 boyut
...
save("stack2_oof", bag_oof)
save("stack2_test", bag_test)
print("\nZENGIN STACK-BAG lb-tahmini:",
      round(mean_squared_error(y, bag_oof, sample_weight=sw), 3))
print("referans: stackbag (sade girdi) 82.615")
