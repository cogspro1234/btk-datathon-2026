# =====================================================================
# Datathon 2026 - stack3: stacker girdisine HAM e5 embedding (SVD128) eklenmis
# Girdi: bilesen tahminleri + 45 ham tabular + e5_svd128 (text manifoldu dogrudan)
...
# e5_train/test.npy Drive'dan (datathon_night/) okunur.
# =====================================================================
import os, shutil
import numpy as np
import pandas as pd
...
Xtr = np.column_stack([np.column_stack(list(Ctr.values())), Rtr, Etr]).astype(np.float32)
Xte = np.column_stack([np.column_stack(list(Cte.values())), Rte, Ete]).astype(np.float32)
print("stack3 girdisi:", Xtr.shape)  # ~19 + 45 + 128
...
save("stack3_oof", bag_oof); save("stack3_test", bag_test)
print("\nstack3 (e5-ham SVD128) lb-tahmini:",
      round(mean_squared_error(y, bag_oof, sample_weight=sw), 3))
print("referans: stack2 (e5 tahmin) 82.515")
